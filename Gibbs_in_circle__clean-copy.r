...
start <- function(N_points, r, d){
  ...
  for (i in 1:N_points){
    ...
    while (k == 0){
      ...
      c=c+1
      if(c==10000) #If we cant generate point for 10 000 times - break
      {
    ...
    #if we didn't get into the previous if conditions => our 2ond point is correct and then k = 1 
    #(and if we get into this conditions then break operator will stop this shit and there will be selected new x2, y2)
        k = 1
      }
    }
    start_samp[c(i), 1] <- x2
...
#------------------
  ...
  names(df) = c("distance", "sample")
  ggplot(df, aes(distance, fill = sample)) + geom_histogram(color="#e9ecef",alpha = 0.6, position = "identity")+scale_fill_manual(values=c("#d35400", "#5499c7"))
  ggplot(df, aes(distance, fill = sample)) + geom_density(color="#e9ecef",alpha = 0.6)+scale_fill_manual(values=c("#d35400", "#5499c7"))
  ...
  names(dist1_NO) = "dist"
  names(dist2_NO) = "dist"
  dist1_NO$name = "point 1"
  dist2_NO$name = "point 2"
  
  dff = rbind(dist1_NO, dist2_NO)
  names(dff) = c("distance", "sample")
  ggplot(dff, aes(distance, fill = sample)) + geom_histogram(color="#e9ecef",alpha = 0.6, position = "identity")+scale_fill_manual(values=c("#d35400", "#5499c7"))
  ggplot(dff, aes(distance, fill = sample)) + geom_density(color="#e9ecef",alpha = 0.6)+scale_fill_manual(values=c("#d35400", "#5499c7"))
